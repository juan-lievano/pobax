...
def choose_goal2d(size: int, key):
    ...
    draws = jax.random.bernoulli(k_draw, 0.5, (2,))  # pick 2 booleans

    r  = jax.random.randint(k_r, (), 0, size)  # random row
    c  = jax.random.randint(k_c, (), 0, size)  # random column
    # a  = jax.random.randint(k_a, (), -(size - 1), size)   # anti-diagonal offset
    # d  = jax.random.randint(k_d, (), 0, 2 * size - 1)     # diagonal offset
...
class EasyLightBulbs2D(Environment):
    """
