...
def uniform_baseline_tensor(grid_size: int) -> np.ndarray:
    # Uniform over interior states/directions EXCEPT the goal state (y_mid, x=1, dir=W=3)
    gs = grid_size
    base = np.zeros((gs, gs, 4), dtype=np.float64)
    base[1:gs-1, 1:gs-1, :] = 1.0
    y_goal = (gs - 1) // 2
    base[y_goal, 1, 3] = 0.0  # exclude the goal state
...
def main():
    ...
    p = argparse.ArgumentParser(description="Linear probe (Ridge): rnn_hidden -> belief tensor")
    ...
    ridge = Ridge(alpha=args.l2, random_state=0)
    ...
    out_root = Path("memory_probe_results_linear")
    ...
    enriched_name = f"test_with_linear_predictions__{Path(args.csv).name}"
